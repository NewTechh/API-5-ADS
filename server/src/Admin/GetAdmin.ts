...
function ListarTodosAdmin(): express.Router{
    const router = express.Router();
...
function ListarAdminID(): express.Router {
    ...
    router.get('/Administradores/:administrador_id', async (req, res) => {
        try {
          ...
          const result = await DB.query('SELECT * FROM Parceiros WHERE parceiro_id = $1', [administrador_id]);
...
export default ListarTodosUsuarios; ListarTodosAdmin;ListarAdminID;